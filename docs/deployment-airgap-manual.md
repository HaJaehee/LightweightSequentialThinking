...
| 승인 페이지가 안 보임 | 브라우저가 **다른 모니터나 뒤쪽 창**에 열렸을 수 있음 (실제 사례) | stderr의 `Blocking for human approval ... at http://127.0.0.1:<port>/` 줄에서 주소 확인. 탭을 띄워두면 1.5초마다 폴링하므로 요청이 오는 즉시 표시됩니다 |
