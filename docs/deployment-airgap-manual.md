...
**서두르지 않으셔도 됩니다.** 대부분의 LLM 클라이언트는 도구 호출 하나를 60초에 강제로
끊고 결과를 버리기 때문에, 서버는 45초마다 스스로 호출을 마무리하고 에이전트에게 곧바로
다시 물어보게 합니다. 사람 입장에서는 아무 차이가 없습니다 — **승인 창은 응답하실 때까지
사라지지 않고**, 누르시면 보통 45초 안에 대화가 저절로 이어집니다(전체 대기 상한은 15분).

각 요청 카드에 표시되는 상태를 보시면 됩니다.

- **에이전트가 대기 중입니다** — 결정하시면 바로 이어집니다.
- **에이전트가 대기를 멈췄습니다** — 결정은 그대로 반영되지만(감사 로그
  `late_decision_applied`), 채팅에 아무 메시지나 한 번 보내셔야 에이전트가 이어서
  진행합니다.

작성 중이던 의견은 다른 세션의 승인 요청이 새로 도착해도, 새로고침을 하셔도, 창을 닫았다
다시 여셔도 남아 있습니다. 탭을 여러 개 열어두셨다면 어느 쪽에 쓰셔도 모든 탭에 똑같이
반영됩니다.

단, 그사이 계획이 바뀌었다면 사람이 본 적 없는 계획에 승인이 적용되지 않도록 그 결정은
폐기됩니다.
...
> **1.8.0부터 계획도 다중화되었습니다.** 여러 세션이 각자의 계획을 동시에 진행할 수
> 있고 서로를 밀어내지 않습니다. `plan_and_think`는 `goal`로 라우팅하며(프롬프트가 매
> 스텝 같은 goal을 반복하도록 지시), 나머지 도구는 활성 계획이 하나면 그대로, 여럿이면
> 선택 파라미터 `plan_id`로 지정합니다 — 여럿일 때는 서버가 `next_action_hint`에
> plan_id를 박아 넣으므로 모델이 힌트를 베끼기만 해도 올바르게 라우팅됩니다.
> 승인도 큐라서 두 세션이 동시에 대기하면 페이지에 2건이 함께 뜹니다.
> 동시 진행 상한은 `PLANNING_MCP_MAX_ACTIVE_PLANS`(기본 5)입니다.
>
> 아래 내용은 **1.7.0 이전** 단일 슬롯 시절의 동작이며, 이제는 같은 goal로 들어온
> 재계획에만 해당합니다.

**(1.7.0 이전) 이 서버의 활성 계획 슬롯은 하나입니다.** 같은 state 디렉터리를 쓰는 모든 대화·워크스페이스가
